"""Real PostgreSQL integration tests for the Market Research API.
...
about proving persistence, the API contract, and the "report survives
even if the profile it was built from later disappears" design decision
are correct, none of which need a real model or a real web search.
Whether the real LLM produces genuinely useful, well-grounded research,
and whether force_regenerate produces genuinely different content
against a real model, is covered separately in
test_market_research_llm_integration.py against real Ollama Cloud,
...
def test_get_before_generation_returns_404(
    ...
    workspace_id = _create_workspace_with_profile(client, "Pre-Generation Research Co")

    try:
        response = client.get(f"/api/v1/workspaces/{workspace_id}/market-research")
...
def test_generate_creates_and_persists_a_report(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "Generate Research Co")
    ...
    try:
        ...
        assert generated["market_overview"]
        assert generated["target_customer_segments"]
        assert generated["competitive_landscape"]
        assert generated["opportunities_and_risks"]
...
def test_generate_without_force_regenerate_returns_existing_record(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "Idempotent Research Co")
    url = f"/api/v1/workspaces/{workspace_id}/market-research"
...
def test_force_regenerate_reuses_the_same_row(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "Regenerate Research Co")
    url = f"/api/v1/workspaces/{workspace_id}/market-research"
...
def test_generate_without_a_business_profile_returns_404(
    ...
    workspace_response = client.post(
        "/api/v1/workspaces",
        json={"name": "No Profile Research Co"},
    )
    assert workspace_response.status_code == 201
    workspace_id = UUID(workspace_response.json()["id"])
...
def test_generate_for_missing_workspace_returns_404(
    ...
    response = client.post(
        f"/api/v1/workspaces/{UUID(int=0)}/market-research",
        json={},
    )
    assert response.status_code == 404
...
def test_get_survives_the_business_profile_being_deleted(
    ...
    Deliberately different from Business Understanding: MarketResearch is
    keyed by workspace_id, not profile_id, and generate()/get() were
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "Orphaned Profile Research Co")
    url = f"/api/v1/workspaces/{workspace_id}/market-research"
...
def test_generate_populates_sources_from_seeded_knowledge_base(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "Seeded Knowledge Research Co")
    url = f"/api/v1/workspaces/{workspace_id}/market-research"
    ...
    try:
        ...
        with database.session() as session:
            RetrievalService(session, embeddings).add_chunk(
                workspace_id,
                "internal:pricing-doc",
                "Our rockets start at $2M per launch.",
            )
        database.dispose()
        ...
        for source in generated["sources"]:
            ...
            assert source["source"] == "internal:pricing-doc"
    finally:
        cleanup_workspace(settings, workspace_id)
