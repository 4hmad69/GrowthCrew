"""Real PostgreSQL integration tests for the Marketing Strategy API.
...
def test_get_before_generation_returns_404(
    ...
    """No strategy exists yet, so GET should 404 with a clear message."""

    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Pre-Generation Strategy Co")

    try:
        response = client.get(f"/api/v1/workspaces/{workspace_id}/marketing-strategy")
...
def test_generate_creates_and_persists_a_report(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Generate Strategy Co")
    ...
    try:
        ...
        assert generated["recommended_channels_and_tactics"]
        assert generated["content_and_messaging_pillars"]
        assert generated["ninety_day_roadmap"]
        assert generated["budget_allocation_and_kpis"]
...
def test_generate_without_force_regenerate_returns_existing_record(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Idempotent Strategy Co")
    url = f"/api/v1/workspaces/{workspace_id}/marketing-strategy"
...
def test_force_regenerate_reuses_the_same_row(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Regenerate Strategy Co")
    url = f"/api/v1/workspaces/{workspace_id}/marketing-strategy"
...
def test_generate_without_a_business_profile_returns_404(
    ...
    workspace_response = client.post(
        "/api/v1/workspaces",
        json={"name": "No Profile Strategy Co"},
    )
    assert workspace_response.status_code == 201
    workspace_id = UUID(workspace_response.json()["id"])
...
def test_generate_without_business_understanding_returns_404(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "No Understanding Strategy Co")
...
def test_generate_without_market_research_returns_404(
    ...
    """A workspace missing only Market Research should 404, not 500."""

    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "No Research Strategy Co")
    _generate_business_understanding(client, workspace_id)
...
def test_generate_without_competitor_analysis_returns_404(
    ...
    """A workspace missing only Competitor Analysis should 404, not 500."""

    client, settings = integration_context
    workspace_id = _create_workspace_with_profile(client, "No Analysis Strategy Co")
    _generate_business_understanding(client, workspace_id)
    _generate_market_research(client, workspace_id)
...
def test_generate_for_missing_workspace_returns_404(
    ...
    response = client.post(
        f"/api/v1/workspaces/{UUID(int=0)}/marketing-strategy",
        json={},
    )
    assert response.status_code == 404
...
def test_get_survives_prerequisite_reports_being_deleted(
    integration_context: tuple[TestClient, Settings],
) -> None:
    """A generated strategy stays readable even after everything it was built from is gone.
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Orphaned Strategy Co")
    url = f"/api/v1/workspaces/{workspace_id}/marketing-strategy"
...
def test_generate_populates_sources_from_seeded_knowledge_base(
    ...
    client, settings = integration_context
    workspace_id = _create_workspace_with_full_prerequisites(client, "Seeded Knowledge Strategy Co")
    url = f"/api/v1/workspaces/{workspace_id}/marketing-strategy"
    ...
    try:
        ...
        with database.session() as session:
            RetrievalService(session, embeddings).add_chunk(
                ...
                "Instagram Reels and an email nurture sequence outperform paid ads for this brand.",
            )
        database.dispose()
        ...
        for source in generated["sources"]:
            ...
            assert source["source"] == "internal:strategy-notes"
    finally:
        cleanup_workspace(settings, workspace_id)
